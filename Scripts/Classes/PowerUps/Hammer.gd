...
const FIREBALL = preload("res://Instances/Prefabs/Projectiles/projectile_hammer.tscn")
...
func main(delta: float) -> void:
	...
	if player.spin_jumping and spin_fire_meter >= 0.5:
		spin_fire_meter = 0
...
func fire_ball(throw_direction := 1) -> void:
	...
	fire_ball_node.global_position = player.global_position - Vector2(throw_direction * -8, 24)
	...
	fire_ball_node.velocity.x = player.velocity.x * 4
